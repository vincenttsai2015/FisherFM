# CODE FROM: https://github.com/HannesStark/dirichlet-flow-matching/blob/main/utils/flow_utils.py
import copy
import math
import pickle
...
def sample_cond_prob_path(mode, fix_alpha, alpha_scale, seq, alphabet_size):
    ...
    if mode == 'dirichlet':
        ...
        if fix_alpha:
            alphas = torch.ones(B, device=seq.device) * fix_alpha
